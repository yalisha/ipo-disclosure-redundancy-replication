...
DATE_TAG = "20260708"
OUT_DIR = ROOT / f"results/glm4_dewrap_join_glm_next100_table2_{DATE_TAG}"
DOC_OUT = ROOT / f"docs/00_current/glm_next100_table2_source_{DATE_TAG}.md"

TABLE2_MASTER = ROOT / "results/cutoff552_table2_471_probe_20260707/table2_471_candidate_master_20260707.csv"
GLM100_CHUNK = ROOT / "results/siliconflow_glm4_32b_glm100_merged_20260707/chunk_metrics_glm4_cot_v3b_len132_tight_20260707.csv"
...
def choose_codes() -> pd.DataFrame:
    ...
    ran = pd.read_csv(GLM100_CHUNK, dtype={"sec_code": str}, encoding="utf-8-sig", usecols=["sec_code"])
...
def order_dataframes(sample: pd.DataFrame, frames: list[pd.DataFrame]) -> None:
...
def write_source_run(
    ...
    manifest = {
        ...
        "purpose": "next 100 Table2 firms not yet run by SiliconFlow GLM-4-32B GLM100",
        "prompt_mode": PROMPT_MODE,
        "firm_n": int(sample["code"].nunique()),
        "chunk_n": int(chunks.shape[0]),
        "source_dirs": [p.name for p in SOURCE_DIRS],
        "selection_rule": "Table2 471 firms excluding existing GLM100 codes, sorted by first_trade_date then code, take next 100",
        "suffix": suffix,
    }
...
def main() -> None:
    ...
    order_dataframes(sample, [chunks, sections, llm])
    ...
    for i in range(SHARD_N):
        ...
        order_dataframes(shard_sample, [shard_chunks, shard_sections, shard_llm])
        shard_dir = ROOT / f"results/glm4_dewrap_join_glm_next100_table2_shard{i + 1}_{DATE_TAG}"
    ...
    doc = [
        ...
        "## Shards",
        "",
        *md_table(shard_plan, ["shard", "source_run_name", "firm_n", "chunk_n", "first_code", "last_code"]),
        "",
        "## 公司清单",
        "",
