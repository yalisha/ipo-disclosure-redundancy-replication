...
ANCHOR = ROOT / "results/glm4_dewrap_join_pilot50_20260705"
OUT = ROOT / "results/glm4_dewrap_join_pilot100_20260705"
PLAN_DIR = ROOT / "results/glm4_dewrap_join_pilot100_20260705_shards"
...
def assign_shards(sample_sections: pd.DataFrame, sample_chunks: pd.DataFrame, seeded_ids: set[str]) -> pd.DataFrame:
    remaining = sample_chunks[~sample_chunks["custom_id"].astype(str).isin(seeded_ids)].copy()
    firm_counts = (
    ...
    year_map = sample_sections.set_index("sec_code")["year"].to_dict()
    plan["year"] = plan["sec_code"].map(year_map)
    return plan


def write_shard_runs(sample_sections: pd.DataFrame, sample_chunks: pd.DataFrame, plan: pd.DataFrame) -> None:
    for shard in range(1, SHARD_N + 1):
        run_name = f"glm4_dewrap_join_pilot100_20260705_shard{shard:02d}"
        shard_dir = ROOT / "results" / run_name
        if shard_dir.exists():
            shutil.rmtree(shard_dir)
        shard_dir.mkdir(parents=True)
        codes = plan.loc[plan["shard"].eq(shard), "sec_code"].astype(str).tolist()
        shard_sections = sample_sections[sample_sections["sec_code"].astype(str).isin(codes)].copy()
        shard_chunks = sample_chunks[sample_chunks["sec_code"].astype(str).isin(codes)].copy()
...
def main() -> None:
    ...
    plan = assign_shards(sample_sections, sample_chunks, seeded_ids)
    write_shard_runs(sample_sections, sample_chunks, plan)
    ...
    (OUT / "sample_codes_20260705.txt").write_text(",".join(sample_sections["sec_code"].astype(str)), encoding="utf-8")
    ...
    manifest = {
        ...
        "seeded_chunks_from_pilot50": int(seeded_chunks),
