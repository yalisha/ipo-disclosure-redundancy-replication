...
DATE_TAG = "20260708"
OUT_RUN = ROOT / f"results/siliconflow_glm4_32b_glm300_merged_{DATE_TAG}"
OUT_DIR = ROOT / f"results/glm300_tailmerge_floor_candidates_{DATE_TAG}"
DOC = ROOT / f"docs/00_current/siliconflow_glm4_32b_glm300_{DATE_TAG}.md"

SOURCES = [
    (
        "glm200",
        ROOT / f"results/siliconflow_glm4_32b_glm200_merged_{DATE_TAG}/chunk_metrics_glm4_cot_v3b_len132_tight_{DATE_TAG}.csv",
    ),
    (
        "table2_next100_after200",
        ROOT / f"results/siliconflow_glm4_32b_table2_next100_after200_merged_{DATE_TAG}/chunk_metrics_glm4_cot_v3b_len132_tight_{DATE_TAG}.csv",
    ),
]
...
def write_doc(helper, chunks: pd.DataFrame, summary: pd.DataFrame) -> None:
    ...
    doc = [
        ...
        "## 结论",
        "",
        f"- 合并样本：firm={chunks['sec_code'].nunique()}，chunk={chunks.shape[0]}，glm200={batch_counts.get('glm200', 0)}，table2_next100_after200={batch_counts.get('table2_next100_after200', 0)}。",
        ...
        f"- GLM300 最接近原文 Table 1 的候选是 `{best_name}`：Summary_len mean={helper.fmt(best['Summary_len_mean'])}，Redundancy_chunk mean={helper.fmt(best['Redundancy_chunk_mean'])}，企业层 Redundancy mean={helper.fmt(best['Redundancy_mean'])}。",
