...
SHARD_RUNS = [f"siliconflow_glm4_32b_table2_next100_shard{i}_{DATE_TAG}" for i in range(1, 6)]
RUN_DIR = ROOT / f"results/siliconflow_glm4_32b_table2_next100_merged_{DATE_TAG}"
OUT_DIR = ROOT / f"results/glm_next100_tailmerge_floor_candidates_{DATE_TAG}"
DOC = ROOT / f"docs/00_current/siliconflow_glm4_32b_table2_next100_{DATE_TAG}.md"
PREV_GLM100 = ROOT / "results/glm100_tailmerge_floor_candidates_20260707/candidate_summary_20260707.csv"
...
def load_helper():
    ...
    module.DOC = DOC
    module.PREV_SUMMARY = PREV_GLM100
    return module


def fmt(helper, value: object) -> str:
...
def write_doc(helper, chunks: pd.DataFrame, summary: pd.DataFrame) -> None:
    ...
    prev_line = "未读取到 GLM100 候选 summary。"
    if PREV_GLM100.exists():
        prev = pd.read_csv(PREV_GLM100).sort_values("loss_all").iloc[0]
        prev_line = (
            f"GLM100 最佳候选为 `{prev['name']}`，"
            f"Summary_len mean={fmt(helper, prev['Summary_len_mean'])}，"
            f"Redundancy_chunk mean={fmt(helper, prev['Redundancy_chunk_mean'])}。"
        )
    ...
    for metric in ["Chunk_num", "Text_len", "Summary_len", "Redundancy_chunk", "lnN_tech", "Redundancy"]:
        ...
        rows.append(
            {
                "metric": metric,
                "mean": fmt(helper, best[f"{metric}_mean"]),
                "paper_mean": fmt(helper, target["mean"]),
                "std": fmt(helper, best[f"{metric}_std"]),
                "paper_std": fmt(helper, target["std"]),
                "median": fmt(helper, best[f"{metric}_median"]),
                "paper_median": fmt(helper, target["median"]),
                "p75": fmt(helper, best[f"{metric}_p75"]),
                "paper_p75": fmt(helper, target["p75"]),
            }
        )
    ...
    doc = [
        ...
        "## 结论",
        "",
        f"- 本轮新增 100 家已跑完并合并：chunk={chunks.shape[0]}，firm={chunks['sec_code'].nunique()}。",
        f"- 原始 GLM proxy：Summary_len mean={fmt(helper, raw['Summary_len_mean'])}，Redundancy_chunk mean={fmt(helper, raw['Redundancy_chunk_mean'])}。",
        f"- next100 最接近原文 Table 1 的候选是 `{best_name}`：Summary_len mean={fmt(helper, best['Summary_len_mean'])}，Redundancy_chunk mean={fmt(helper, best['Redundancy_chunk_mean'])}，企业层 Redundancy mean={fmt(helper, best['Redundancy_mean'])}。",
        f"- {prev_line}",
...
def main() -> None:
    ...
    helper.merge_shards()
    chunks = helper.build_chunk_metrics()
